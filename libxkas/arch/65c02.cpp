// WDC 65c02 / 65sc02 assembly
// based on 6502.cpp
// "header"/"noheader" directives were removed since I don't know of a 
// common platform for this arch that would need it.

void xkas65c02::init(unsigned pass) {
	bank = 0;
	bank_size = 0x10000;
}

unsigned xkas65c02::archaddr(unsigned addr) {
...
unsigned xkas65c02::fileaddr(unsigned addr) {
	// assume the address is within the current bank
	addr = (bank * bank_size) + (addr % bank_size);
	return addr;
}

bool xkas65c02::assemble_command(string &s) {
	part.qsplit<1>(" ", s);
	part[0].lower();
	part[0].trim(" ");
...
bool xkas65c02::assemble_direct() {
...
bool xkas65c02::assemble_indirect_x() {
	if(force_word) return false;
	#define match(str_, hex_) \
...
bool xkas65c02::assemble_indirect_y() {
	if(force_word) return false;
	#define match(str_, hex_) \
		if(part[0] == str_) { \
			unsigned n = self.decode(part[1]); \
...
bool xkas65c02::assemble_indirect() {
	#define matchb(str_, hex_) \
		if(part[0] == str_ && !force_word) { \
			unsigned n = self.decode(part[1]); \
			self.write(hex_); \
...
bool xkas65c02::assemble_addr_x() {
...
bool xkas65c02::assemble_addr_y() {
	if(force_byte) return false;
	#define match(str_, hex_) \
		if(part[0] == str_) { \
			unsigned n = self.decode(part[1]); \
...
bool xkas65c02::assemble_branch() {
	#define match(str_, hex_) \
		if(part[0] == str_) { \
...
xkas65c02::xkas65c02(xkas &self) : xkasArch(self) {
}
